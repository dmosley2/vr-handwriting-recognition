...
{
    public static Texture2D DrawingCircle(this Texture2D tex, int x, int y, int r, Color color)
    {
        float rSquared = r * r;
...
public class HapticDrawing : MonoBehaviour, IBoard
{
    ...
    //Create new texture of defined size and make it white
    public void ClearTexture()
    {
        ...
        for (int u = 0; u < newTexture.width; u++)
        {
            for (int v = 0; v < newTexture.height; v++)
            {
                newTexture.SetPixel(u, v, Color.white);
            }
        }
        newTexture.Apply();
        gameObject.GetComponent<MeshRenderer>().sharedMaterial.mainTexture = newTexture;
    }
    ...
    private void OnCollisionStay(Collision collision)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (bumpMap != null)
            {
                ...
                bumpMap.DrawingCircle((int)pixelUV.x, (int)pixelUV.y, Size, DrawColor);
                bumpMap.Apply();
    ...
    {
        Size = Mathf.Min(Size + 1, 12);
    }

    public void decreaseSize() {
        Size = Mathf.Max(Size - 1, 4);
    }
